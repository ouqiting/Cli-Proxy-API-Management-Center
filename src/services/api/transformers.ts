...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  const upstreamTimeout = raw['upstream-timeout'] ?? raw.upstreamTimeout;
  if (typeof upstreamTimeout === 'number' && Number.isFinite(upstreamTimeout)) {
    config.upstreamTimeout = upstreamTimeout;
  } else if (typeof upstreamTimeout === 'string' && upstreamTimeout.trim() !== '') {
    const parsed = Number(upstreamTimeout);
    if (Number.isFinite(parsed)) {
      config.upstreamTimeout = parsed;
    }
  }
